...
namespace Dollie\SDK\Integrations\Academylms\Actions;
...
class AcEnrollUserToCourse extends AutomateAction
{
    ...
    public function register($actions)
    {
        $actions[$this->integration][$this->action] = [
            'label' => __('Enroll User To Course', 'dollie'),
            'action' => $this->action,
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        ...
        if (! class_exists('\Academy\Helper')) {
            return [
                'status' => 'error',
                'message' => __('\Academy\Helper class not found.', 'dollie'),
            ];
        }
        ...
        if (is_email($user_id)) {
            ...
            if ($user) {
                ...
                if (! $is_enrolled) {
                    ...
                    if ($enrolled) {
                        ...
                        $error = [
                            ...
                            'response' => esc_attr__('Something went wrong.', 'dollie'),
                        ];
                    ...
                    $error = [
                        'status' => esc_attr__('Error', 'dollie'),
                        'response' => esc_attr__('User is already enrolled to course.', 'dollie'),
                    ];
                ...
                $error = [
                    'status' => esc_attr__('Error', 'dollie'),
                    'response' => esc_attr__('User not exists.', 'dollie'),
                ];
