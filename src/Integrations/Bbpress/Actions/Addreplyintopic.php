<?php
namespace Dollie\SDK\Integrations\Bbpress\Actions;
...
#[Action(
    id: 'bbp_add_reply_in_topic',
    label: 'Add Reply In Topic',
    since: '1.0.0'
)]
...
class AddReplyInTopic extends AutomateAction {
	...
	public function register( $actions ) {
		$actions[ $this->integration ][ $this->action ] = [
			'label'    => __( 'Add Reply In Topic', 'dollie' ),
			'action'   => $this->action,
	...
	public function _action_listener( $user_id, $automation_id, $fields, $selected_options ) {
		if ( ! class_exists( 'bbPress' ) ) {
			return [
				'status'  => 'error',
				'message' => __( 'bbPress class not found.', 'dollie' ), 
			];
		}
