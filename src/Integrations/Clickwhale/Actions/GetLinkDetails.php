...
#[Action(
    id: 'cw_get_link_details',
    label: 'Get Link Details',
    since: '1.0.0'
)]
/**
 * GetLinkDetails.
...
class GetLinkDetails extends AutomateAction
{
    ...
    public function register($actions)
    {
        $actions[$this->integration][$this->action] = [
            'label' => __('Get Link Details', 'dollie'),
            'action' => $this->action,
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        ...
        if (empty($link_id) && empty($slug)) {
            return [
                'status' => 'error',
                'message' => __('Either Link ID or Slug is required to get link details.', 'dollie'),
        ...
        if ($link_id) {
            ...
            if (! $link) {
                return [
                    'status' => 'error',
                    'message' => __('Invalid Link ID.', 'dollie'),
            ...
            if (! $link) {
                return [
                    'status' => 'error',
                    'message' => __('Invalid Slug.', 'dollie'),
