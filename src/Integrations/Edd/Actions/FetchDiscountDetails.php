...
#[Action(
    id: 'edd_fetch_discount_details',
    label: 'Fetch Discount Details',
    since: '1.0.0'
)]
/**
...
class FetchDiscountDetails extends AutomateAction
{
    ...
    public function register($actions)
    {
        $actions[$this->integration][$this->action] = [
            'label' => __('Fetch Discount Details', 'dollie'),
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        if (! function_exists('edd_get_discount_by')) {
            return [
                'status' => 'error',
                'message' => __('EDD plugin is not active.', 'dollie'),
        ...
        if (empty($discount_identifier)) {
            return [
                'status' => 'error',
                'message' => __('Discount code or ID is required.', 'dollie'),
        ...
        if (! $discount) {
            return [
                'status' => 'error',
                'message' => __('Discount not found.', 'dollie'),
