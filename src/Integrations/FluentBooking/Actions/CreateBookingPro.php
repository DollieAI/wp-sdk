...
#[Action(
    id: 'fluent_booking_create_booking_pro',
    label: 'Create Booking (Pro)',
    since: '1.0.0'
)]
/**
 * CreateBookingPro.
...
class CreateBookingPro extends AutomateAction
{
    ...
    public function register($actions)
    {
        $actions[$this->integration][$this->action] = [
            'label' => __('Create Booking (Pro)', 'dollie'),
            'action' => $this->action,
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        ...
        if (! defined('FLUENT_BOOKING_PRO_VERSION')) {
            return [
                'status' => 'error',
                'message' => __('FluentBooking Pro is required for this action.', 'dollie'),
            ];
        }
        ...
        if (! class_exists('FluentBooking\App\Services\BookingService')) {
            return [
                'status' => 'error',
                'message' => __('FluentBooking BookingService class not found.', 'dollie'),
            ];
        }
        ...
        if (! class_exists('FluentBooking\App\Models\CalendarSlot')) {
            return [
                'status' => 'error',
                'message' => __('FluentBooking CalendarSlot model not found.', 'dollie'),
            ];
        }
        ...
        if (! class_exists('FluentBooking\App\Services\DateTimeHelper')) {
            return [
                'status' => 'error',
                'message' => __('FluentBooking DateTimeHelper service not found.', 'dollie'),
            ];
        }
        ...
        if (! class_exists('FluentBooking\App\Services\Helper')) {
            return [
                'status' => 'error',
                'message' => __('FluentBooking Helper service not found.', 'dollie'),
            ];
        }
        ...
        if (empty($event_id) || empty($name) || empty($email) || empty($start_time)) {
            return [
                'status' => 'error',
                'message' => __('Event ID, Name, Email, and Start Time are required.', 'dollie'),
            ];
        }
        ...
        if (! filter_var($email, FILTER_VALIDATE_EMAIL)) {
            return [
                'status' => 'error',
                'message' => __('Invalid email address provided.', 'dollie'),
            ];
        }
        ...
        try {
            ...
            if (! $calendar_slot) {
                return [
                    'status' => 'error',
                    'message' => sprintf(__('Calendar event not found with ID: %d', 'dollie'), $event_id),
                ];
            }
            ...
            if ('active' !== $calendar_slot->status) {
                return [
                    'status' => 'error',
                    'message' => __('This calendar event is not accepting bookings.', 'dollie'),
                ];
            }
            ...
            return [
                'status' => 'error',
                'message' => __('Failed to create booking: ', 'dollie') . $e->getMessage(),
            ];
        }
    }
    ...
    private function generate_google_meet_link($booking_data, $user_id)
    {
        try {
            ...
            $event_data = [
                'summary' => sprintf(__('Meeting with %s', 'dollie'), $booking_data['name']),
    ...
    private function generate_zoom_meeting_link($booking_data, $user_id)
    {
        try {
            ...
            $meeting_data = [
                'topic' => sprintf(__('Meeting with %s', 'dollie'), $booking_data['name']),
                'type' => 2,
