...
#[Action(
    id: 'fluent_booking_get_booking_details',
...
class GetBookingDetails extends AutomateAction
{
    ...
    public function register($actions)
    {
        $actions[$this->integration][$this->action] = [
            'label' => __('Get Booking Details', 'dollie'),
            'action' => $this->action,
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        ...
        if (empty($booking_id) && empty($booking_email) && empty($booking_hash)) {
            return [
                'status' => 'error',
                'message' => __('At least one booking identifier (ID, email, or hash) is required.', 'dollie'),
            ];
        }
        ...
        try {
            ...
            if (! $booking) {
                return [
                    'status' => 'error',
                    'message' => __('Booking not found with the provided identifiers.', 'dollie'),
                ];
            }
            ...
            return [
                'status' => 'error',
                'message' => __('Failed to get booking details: ', 'dollie') . $e->getMessage(),
            ];
        }
    }
}
