...
#[Action(
    id: 'fluent_booking_update_booking_status',
...
class UpdateBookingStatus extends AutomateAction
{
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        ...
        if (empty($booking_id) || empty($new_status)) {
            return [
                'status' => 'error',
                'message' => __('Booking ID and Status are required.', 'dollie'),
            ];
        }
        ...
        if (! in_array($new_status, $valid_statuses, true)) {
            return [
                'status' => 'error',
                'message' => sprintf(__('Invalid booking status. Valid statuses are: %s', 'dollie'), implode(', ', $valid_statuses)),
            ];
        }
        ...
        try {
            ...
            return [
                'status' => 'error',
                'message' => __('Failed to update booking status: ', 'dollie') . $e->getMessage(),
            ];
        }
    }
}
