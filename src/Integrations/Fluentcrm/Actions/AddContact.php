...
class AddContact extends AutomateAction
{
    ...
    public function register($actions)
    {
        ...
        $actions[$this->integration][$this->action] = [
            'label' => __('Add/Update Contact', 'dollie'),
            'action' => $this->action,
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        if (! function_exists('FluentCrmApi') || ! function_exists('fluentcrm_get_custom_contact_fields')) {
            return [
                'status' => 'error',
                'message' => __('FluentCRM functions not found.', 'dollie'),
        ...
        if (empty($selected_options['contact_email']) || ! is_email($selected_options['contact_email'])) {
            return [
                'status' => 'error',
                'message' => __('Email address is invalid.', 'dollie'),
        ...
        if ('' !== $dob) {
            ...
            if (! $date_of_birth) {
                return [
                    'status' => 'error',
                    'message' => __("The date format does not conform to the 'yyyy-mm-dd' format in Date of Birth field.", 'dollie'),
            ...
            foreach ($selected_options['field_row_repeater'] as $key => $field) {
                ...
                if (in_array($type, ['select-one', 'radio'], true)) {
                    ...
                    if (! $field_value) {
                        return [
                            'status' => 'error',
                            'message' => sprintf(__("The value '%1\$s' is not a valid option in the %2\$s field in FluentCRM.", 'dollie'), $value, $label),
                        ];
                    }
                    ...
                    foreach ($option_values as $option_value) {
                        ...
                        if (! $field_value) {
                            return [
                                'status' => 'error',
                                'message' => sprintf(__("The value '%1\$s' is not a valid option in the %2\$s field in FluentCRM.", 'dollie'), $option_value, $label),
                            ];
                        }
                    ...
                    if (! $date) {
                        return [
                            'status' => 'error',
                            'message' => sprintf(__("The date format does not conform to the 'yyyy-mm-dd' format in %s field.", 'dollie'), $label),
                        ];
                    }
                    ...
                    if (! $date) {
                        return [
                            'status' => 'error',
                            'message' => sprintf(__("The datetime format does not conform to the 'yyyy-mm-dd hh:mm:ss' format in %s field.", 'dollie'), $label),
                        ];
                    }
        ...
        if (! empty($selected_tag)) {
            if (is_array($selected_tag)) {
                ...
                if (! class_exists('FluentCrm\App\Models\Tag')) {
                    return [
                        'status' => 'error',
                        'message' => __('Tag model not found.', 'dollie'),
        ...
        if (! $contact) {
            return [
                'status' => 'error',
                'message' => __('Invalid contact.', 'dollie'),
