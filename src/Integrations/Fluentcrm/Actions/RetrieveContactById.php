...
#[Action(
    id: 'fluentcrm_retrieve_contact_by_id',
    label: 'Retrieve Contact By ID',
    since: '1.0.0'
)]
/**
 * RetrieveContactById.
...
class RetrieveContactById extends AutomateAction
{
    ...
    public function register($actions)
    {
        ...
        $actions[$this->integration][$this->action] = [
            'label' => __('Retrieve Contact By ID', 'dollie'),
            'action' => $this->action,
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        ...
        if (is_null($contact)) {
            return [
                'message' => __('Can not find the contact with the ID.', 'dollie'),
                'status' => 'false',
                'user_exists' => 'false',
            ];
        }
