...
#[Action(
    id: 'fluentcrm_retrieve_contact_by_segment',
    label: 'Retrieve Contact By Segment',
    since: '1.0.0'
)]
/**
...
class RetrieveContactBySegment extends AutomateAction
{
    ...
    public function register($actions)
    {
        ...
        $actions[$this->integration][$this->action] = [
            'label' => __('Retrieve Contact By Segment', 'dollie'),
            'action' => $this->action,
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        if (! function_exists('FluentCrmApi')) {
            return [
                'status' => 'error',
                'message' => __('FluentCRM is not active.', 'dollie'),
        ...
        if (empty($segment) || empty($segment['subscribers'])) {
            return [
                'message' => __('No contacts found or segment does not exist', 'dollie'),
                'status' => 'false',
                'user_exists' => 'false',
            ];
        }
        ...
        if (empty($contacts)) {
            return [
                'message' => __('No contacts found', 'dollie'),
                'status' => 'false',
                'user_exists' => 'false',
            ];
        }
