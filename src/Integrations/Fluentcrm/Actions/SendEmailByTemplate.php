...
#[Action(
    id: 'fluentcrm_send_email_by_template',
    label: 'Send Email By Template',
    since: '1.0.0'
)]
/**
 * SendEmailByTemplate.
...
class SendEmailByTemplate extends AutomateAction
{
    ...
    public function register($actions)
    {
        $actions[$this->integration][$this->action] = [
            'label' => __('Send Email By Template', 'dollie'),
            'action' => $this->action,
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        ...
        if (! $template_id) {
            return [
                'status' => 'error',
                'message' => __('Template ID is required.', 'dollie'),
        ...
        if (! $template) {
            return [
                'status' => 'error',
                'message' => __('Template not found.', 'dollie'),
        ...
        if (200 !== $response_code) {
            ...
            $error_message = __('Failed to create campaign.', 'dollie');
            ...
            return [
                'status' => 'error',
                'message' => __('Failed to create campaign.', 'dollie'),
        ...
        if (false === $settings_data) {
            return [
                'status' => 'error',
                'message' => __('Failed to encode campaign settings data.', 'dollie'),
        ...
        if (! is_array($response_context) || ! isset($response_context['id'])) {
            return [
                'status' => 'error',
                'message' => __('Invalid campaign response: missing ID.', 'dollie'),
        ...
        if (200 !== $settings_response_code) {
            return [
                'status' => 'error',
                'message' => __('Failed to update campaign settings.', 'dollie'),
        ...
        if (false === $contact_body) {
            return [
                'status' => 'error',
                'message' => __('Failed to encode contact data.', 'dollie'),
        ...
        if (is_array($contacts_context) && 0 == $contacts_context['count']) {
            return [
                'status' => 'error',
                'message' => __('No contacts found based on your selection.', 'dollie'),
        ...
        if (! is_array($settings_context) || ! isset($settings_context['campaign']) || ! isset($settings_context['campaign']['id'])) {
            return [
                'status' => 'error',
                'message' => __('Invalid campaign ID.', 'dollie'),
        ...
        if (is_wp_error($final_request)) {
            return [
                'status' => 'error',
                'message' => __('Failed to schedule campaign.', 'dollie'),
