...
#[Action(
    id: 'jetpack_crm_change_contact_status',
...
class ChangeContactStatus extends AutomateAction
{
    ...
    public function register($actions)
    {
        ...
        $actions[$this->integration][$this->action] = [
            'label' => __('Change Contact Status', 'dollie'),
            'action' => $this->action,
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        ...
        if (empty($status) || empty($contact_email)) {
            throw new Exception(__('Status or contact email is empty.', 'dollie'));
        }
        ...
        if (! empty($contact) && $status !== $contact->zbsc_status) {
            ...
            return [
                ...
                'success' => true,
                'msg' => __('The contact status has been updated.', 'dollie'),

            ];
        } else {
            throw new Exception(__('Contact was not found matching.', 'dollie'));
        }
    }
}
