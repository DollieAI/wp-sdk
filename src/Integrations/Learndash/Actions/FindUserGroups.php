...
#[Action(
    id: 'learndash_find_user_groups',
    label: 'Find User Groups',
    since: '1.0.0'
)]
/**
 * FindUserGroups.
...
class FindUserGroups extends AutomateAction
{
    ...
    public function register($actions)
    {
        $actions[$this->integration][$this->action] = [
            'label' => __('Find User Groups', 'dollie'),
            'action' => $this->action,
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        ...
        if (! function_exists('learndash_get_users_group_ids')) {
            return [
                'status' => 'error',
                'message' => __('LearnDash function not available', 'dollie'),
        ...
        if (empty($user_groups)) {
            return [
                'message' => __('User is not part of any group', 'dollie'),
