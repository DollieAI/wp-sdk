...
namespace Dollie\SDK\Integrations\Mailpoet\Actions;
...
#[Action(
    id: 'update_subscriber_to_new_list',
...
class UpdateSubscriberToNewList extends AutomateAction
{
    ...
    public function register($actions)
    {
        $actions[$this->integration][$this->action] = [
            'label' => __('Add User to List', 'dollie'),
            'action' => $this->action,
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        if (! class_exists('\MailPoet\API\API') || ! class_exists('\MailPoet\API\MP\v1\APIException')) {
            return [
                'status' => 'error',
                'message' => __('MailPoet API class not exists.', 'dollie'),
        ...
        if ('' === $list_id) {
            return [
                'status' => 'error',
                'message' => __('Please enter a list.', 'dollie'),
