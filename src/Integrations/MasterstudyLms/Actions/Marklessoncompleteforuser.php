...
#[Action(
    id: 'mark_lesson_complete',
    label: 'Mark a lesson complete for the user',
    since: '1.0.0'
)]
...
class MarkLessonCompleteForUser extends AutomateAction {
	...
	public function register( $actions ) {
		$actions[ $this->integration ][ $this->action ] = [
			'label'    => __( 'Mark a lesson complete for the user', 'dollie' ),
			'action'   => $this->action,
	...
	public function _action_listener( $user_id, $automation_id, $fields, $selected_options ) {
		...
		if ( empty( $curriculum_arr ) ) {
			$error = [
				...
				'response' => esc_attr__( 'No Lessons found in selected Course.', 'dollie' ), 
		...
		if ( ! empty( $lessons ) ) {
			...
			if ( ! class_exists( '\STM_LMS_Lesson' ) ) {
				return [
					'status'  => 'error',
					'message' => __( '\STM_LMS_Lesson class not found.', 'dollie' ), 
			...
			$response = [
				...
				'response' => esc_attr__( 'Lesson marked complete successfully.', 'dollie' ), 
				
			];
		} else {
			$error = [
				'status'   => esc_attr__( 'Error', 'dollie' ),
				'response' => esc_attr__( 'Lesson not found.', 'dollie' ), 
