...
class UnrollUserFromCourse extends AutomateAction {
	...
	public function register( $actions ) {
		$actions[ $this->integration ][ $this->action ] = [
			'label'    => __( 'Unroll User From Course', 'dollie' ),
			'action'   => $this->action,
	...
	public function _action_listener( $user_id, $automation_id, $fields, $selected_options ) {
		...
		if ( is_email( $user_id ) ) {
			...
			if ( $user ) {
				...
				$error = [
					...
					'response' => esc_attr__( 'User with the email provided not found.', 'dollie' ), 
		...
		if ( function_exists( 'stm_lms_get_user_course' ) ) {
			...
			if ( empty( $course ) ) {
				$response = [
					'status'   => esc_attr__( 'Error', 'dollie' ),
					'response' => esc_attr__( 'User not enrolled into this course.', 'dollie' ), 
				...
				if ( ! empty( $curriculum_arr ) ) {
					...
					$response = [
						...
						'response' => esc_attr__( 'User progress reset successfully.', 'dollie' ), 
					...
					$error =
						[
							'status'   => esc_attr__( 'Error', 'dollie' ),
							'response' => esc_attr__( 'Something went wrong. Please check the action step configuration.', 'dollie' ), 
				...
				$response = [
					'status'   => esc_attr__( 'Success', 'dollie' ),
					'response' => esc_attr__( 'User unrolled from course successfully.', 'dollie' ), 
