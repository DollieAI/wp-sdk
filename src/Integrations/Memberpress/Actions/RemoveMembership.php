...
namespace Dollie\SDK\Integrations\Memberpress\Actions;
...
#[Action(
    id: 'mp_remove_membership',
    label: 'Remove from Membership',
    since: '1.0.0'
)]
/**
 * RemoveMembership.
...
class RemoveMembership extends AutomateAction
{
    ...
    public function register($actions)
    {
        $actions[$this->integration][$this->action] = [
            'label' => __('Remove from Membership', 'dollie'),
            'action' => $this->action,
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        ...
        if (0 === $table['count']) {
            return [
                'status' => 'error',
                'message' => __('Empty subscription table ', 'dollie'),
