...
#[Action(
    id: 'get_metabox_value',
    label: 'Get Meta Value',
    since: '1.0.0'
)]
/**
 * GetMetaBoxValue.
...
class GetMetaBoxValue extends AutomateAction {
	...
	public function register( $actions ) {
		$actions[ $this->integration ][ $this->action ] = [
			'label'    => __( 'Get Meta Value', 'dollie' ),
			'action'   => $this->action,
	...
	public function _action_listener( $user_id, $automation_id, $fields, $selected_options ) {
		...
		if ( function_exists( 'rwmb_get_value' ) ) {
			...
			if ( empty( $response ) ) {
				$response = [
					'response' => esc_attr__( 'No value found.', 'dollie' ), 
			...
			$response = [
				'Error' => esc_attr__( 'Function rwmb_get_value not exists. Please make sure the Metabox plugin is installed and active.', 'dollie' ), 
