...
#[Action(
    id: 'remove_user_from_membership_level',
    label: 'Remove the user from a membership level',
    since: '1.0.0'
)]
/**
...
class RemoveUserMembershipLevel extends AutomateAction
{
    ...
    public function register($actions)
    {
        $actions[$this->integration][$this->action] = [
            'label' => __('Remove the user from a membership level', 'dollie'),
            'action' => $this->action,
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        ...
        if (! function_exists('pmpro_getMembershipLevelsForUser')) {
            return [
                'status' => 'error',
                'message' => __('pmpro_getMembershipLevelsForUser function not found.', 'dollie'),
        ...
        if (intval('-1') === intval($membership_level)) {
            ...
            if (empty($user_membership_levels)) {
                $error = [
                    ...
                    'response' => esc_attr__('User does not belong to any membership levels.', 'dollie'),
            ...
            $response = [
                ...
                'response' => esc_attr__('User removed from Membership level.', 'dollie'),
        ...
        if (! in_array($membership_level, $user_membership_levels, true)) {
            ...
            $error = [
                'status' => esc_attr__('Error', 'dollie'),
                'response' => esc_attr__('User was not a member of the specified level.', 'dollie'),
        ...
        if (function_exists('pmpro_cancelMembershipLevel')) {
            if (pmpro_cancelMembershipLevel(absint($membership_level), absint($user_id))) {
                ...
                $error = [
                    'status' => esc_attr__('Error', 'dollie'),
                    'response' => esc_attr__("We're unable to cancel the specified level from the user.", 'dollie'),
