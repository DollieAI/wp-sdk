...
namespace Dollie\SDK\Integrations\PrestoPlayer\Actions;
...
#[Action(
    id: 'pp_create_mediahub',
    label: 'Create Media Hub',
    since: '1.0.0'
)]
/**
 * CreateMediaHub.
...
class CreateMediaHub extends AutomateAction
{
    ...
    public function register($actions)
    {
        $actions[$this->integration][$this->action] = [
            'label' => __('Create Media Hub', 'dollie'),
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        ...
        if (empty($video_url) && empty($basic_video_url)) {
            return [
                'message' => __('Both YouTube and self-hosted video URLs are empty. Please provide at least one.', 'dollie'),
        ...
        if (! empty($video_url)) {
            ...
            if (preg_match($pattern, $video_url, $matches)) {
                ...
                return [
                    'message' => __('Invalid YouTube URL.', 'dollie'),
        ...
        if ($id) {
            ...
            return [
                'message' => __('There was an error creating the video!', 'dollie'),
