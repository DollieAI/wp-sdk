...
class AddNewOrder extends AutomateAction
{
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        ...
        ! function_exists('ppress_clean')
            ...
            return [
                'status' => 'error',
                'message' => __('ProfilePress functions not found.', 'dollie'),
        ...
        if (! class_exists('ProfilePress\Core\Membership\Models\Customer\CustomerFactory') ||
            ...
            return [
                'status' => 'error',
                'message' => __('ProfilePress classes not found.', 'dollie'),
        ...
        if (empty($order_data['plan'])) {
            return [
                ...
                'response' => __('No plan was selected. Please try again.', 'dollie'),
        ...
        if (empty($order_data['customer'])) {
            return [
                'status' => __('Error', 'dollie'),
                'response' => __('No customer was selected. Please try again.', 'dollie'),
        ...
        if (! $plan || empty($plan->id)) {
            return [
                'status' => __('Error', 'dollie'),
                'response' => __('The selected plan does not exist.', 'dollie'),
        ...
        if (! $customer || ! $customer->user_exists()) {
            return [
                'status' => __('Error', 'dollie'),
                'response' => __('The selected customer does not exist.', 'dollie'),
        ...
        return [
            'status' => __('Success', 'dollie'),
            'response' => sprintf(
                __('Order successfully created with ID %d.', 'dollie'),
                $response['order_id']
            ),
            'order' => $response,
        ];
