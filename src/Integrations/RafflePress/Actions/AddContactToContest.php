...
namespace Dollie\SDK\Integrations\RafflePress\Actions;
...
#[Action(
    id: 'rafflepress_add_contact_to_contest',
    label: 'Add Contact to Contest',
    since: '1.0.0'
)]
/**
 * AddContactToContest.
...
class AddContactToContest extends AutomateAction
{
    ...
    public function register($actions)
    {
        ...
        $actions[$this->integration][$this->action] = [
            'label' => __('Add Contact to Contest', 'dollie'),
            'action' => $this->action,
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        ...
        if (! defined('RAFFLEPRESS_BUILD') && ! defined('RAFFLEPRESS_PRO_BUILD')) {
            return [
                'status' => 'error',
                'message' => __('RafflePress plugin is not installed or activated.', 'dollie'),
            ];
        }
        ...
        if (empty($contest_id)) {
            return [
                'status' => 'error',
                'message' => __('Contest ID is required.', 'dollie'),
            ];
        }
        ...
        if (empty($email) || ! is_email($email)) {
            return [
                'status' => 'error',
                'message' => __('Valid email address is required.', 'dollie'),
            ];
        }
        ...
        if (! $contest) {
            return [
                'status' => 'error',
                'message' => __('Contest not found.', 'dollie'),
            ];
        }
        ...
        if (! $contest['active']) {
            return [
                'status' => 'error',
                'message' => __('Contest is not active.', 'dollie'),
            ];
        }
        ...
        if ($existing_contestant) {
            return [
                'status' => 'error',
                'message' => __('Contact already exists in this contest.', 'dollie'),
            ];
        }
        ...
        if (false === $inserted) {
            return [
                'status' => 'error',
                'message' => __('Failed to add contact to contest.', 'dollie'),
            ];
        }
