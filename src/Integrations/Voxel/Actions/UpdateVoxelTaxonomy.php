...
#[Action(
    id: 'voxel_update_taxonomy',
    label: 'Update Voxel Taxonomy',
    since: '1.0.0'
)]
/**
 * UpdateVoxelTaxonomy.
...
class UpdateVoxelTaxonomy extends AutomateAction
{
    ...
    public function register($actions)
    {
        $actions[$this->integration][$this->action] = [
            'label' => __('Update Voxel Taxonomy', 'dollie'),
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        ...
        if (empty($taxonomy)) {
            return [
                'success' => false,
                'message' => esc_attr__('Taxonomy is required', 'dollie'),
            ];
        }
        ...
        if (! $post_type_obj) {
            return [
                'success' => false,
                'message' => esc_attr__('Post type not found', 'dollie'),
            ];
        }
    ...
    private function update_wordpress_taxonomy($post_id, $taxonomy, $terms)
    {
        ...
        if (empty($term_ids)) {
            return [
                'success' => false,
                'message' => esc_attr__('No valid terms found', 'dollie'),
            ];
        }
        ...
        if (is_wp_error($result)) {
            return [
                'success' => false,
                'message' => esc_attr__('Failed to update taxonomy: ', 'dollie') . $result->get_error_message(),
            ];
        }

        return [
            'success' => true,
            'message' => esc_attr__('WordPress taxonomy updated successfully', 'dollie'),
            'post_id' => $post_id,
            'taxonomy' => $taxonomy,
    ...
    private function update_voxel_taxonomy_field($post, $post_type_obj, $taxonomy, $terms, $post_id)
    {
        ...
        if (! is_array($post_fields)) {
            return [
                'success' => false,
                'message' => esc_attr__('Unable to get post fields', 'dollie'),
            ];
        }
        ...
        if (! $taxonomy_field) {
            return [
                'success' => false,
                'message' => esc_attr__('Voxel taxonomy field not found', 'dollie'),
            ];
        }
        ...
        if (! $post_field) {
            return [
                'success' => false,
                'message' => esc_attr__('Voxel post field not found', 'dollie'),
            ];
        }
        ...
        try {
            ...
            return [
                'success' => true,
                'message' => esc_attr__('Voxel taxonomy field updated successfully', 'dollie'),
                'post_id' => $post_id,
                'taxonomy' => $taxonomy,
                'terms_updated' => count($term_ids),
                'terms_created' => $created_terms,
            ...
            return [
                'success' => false,
                'message' => esc_attr__('Failed to update Voxel taxonomy: ', 'dollie') . $e->getMessage(),
            ];
        }
    }
}
