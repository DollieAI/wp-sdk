...
#[Action(
    id: 'wte_retrieve_enquiry',
    label: 'Retrieve an Enquiry',
    since: '1.0.0'
)]
/**
 * RetrieveEnquiry.
...
class RetrieveEnquiry extends AutomateAction
{
    ...
    public function register($actions)
    {
        $actions[$this->integration][$this->action] = [
            'label' => __('Retrieve an Enquiry', 'dollie'),
            'action' => $this->action,
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        if (empty($selected_options['enquiry_id'])) {
            return [
                'status' => 'error',
                'message' => __('Enquiry ID is required.', 'dollie'),
        ...
        if (! $post || 'enquiry' !== $post->post_type) {
            return [
                'status' => 'error',
                'message' => __('Enquiry not found.', 'dollie'),
