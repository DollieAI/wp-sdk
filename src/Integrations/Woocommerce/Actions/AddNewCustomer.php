...
#[Action(
    id: 'wc_add_new_customer',
    label: 'Add New Customer in WooCommerce',
    since: '1.0.0'
)]
/**
 * AddNewCustomer.
...
class AddNewCustomer extends AutomateAction
{
    ...
    public function register($actions)
    {
        $actions[$this->integration][$this->action] = [
            'label' => __('Add New Customer in WooCommerce', 'dollie'),
            'action' => $this->action,
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        if (! function_exists('wc_create_new_customer')) {
            return [
                'status' => 'error',
                'message' => __('WooCommerce function wc_create_new_customer not found.', 'dollie'),
        ...
        if (! class_exists('WC_Customer')) {
            return [
                'status' => 'error',
                'message' => __('WooCommerce class WC_Customer not found.', 'dollie'),
        ...
        if (email_exists($email)) {
            return [
                'status' => 'error',
                'message' => __('Email already exists.', 'dollie'),
