...
#[Action(
    ...
    label: 'Create Product in WooCommerce',
    since: '1.0.0'
)]
/**
 * CreateProduct.
...
class CreateProduct extends AutomateAction
{
    ...
    public function register($actions)
    {
        $actions[$this->integration][$this->action] = [
            'label' => __('Create Product in WooCommerce', 'dollie'),
            'action' => $this->action,
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        ...
        if (! $product) {
            return new WP_Error('product_not_found', __('Failed to get the product.', 'dollie'));
        }
        $product->set_props(
            [
