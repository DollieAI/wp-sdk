...
#[Action(
    id: 'wc_fetch_coupon_details',
    label: 'Fetch Coupon Details',
    since: '1.0.0'
)]
/**
 * FetchCouponDetails.
...
class FetchCouponDetails extends AutomateAction
{
    ...
    public function register($actions)
    {
        $actions[$this->integration][$this->action] = [
            'label' => __('Fetch Coupon Details', 'dollie'),
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        ...
        if (empty($coupon_identifier)) {
            return [
                'status' => 'error',
                'message' => __('Coupon code or ID is required.', 'dollie'),
        ...
        if (! $coupon->get_id()) {
            return [
                'status' => 'error',
                'message' => __('Coupon not found.', 'dollie'),
