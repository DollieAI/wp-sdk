...
#[Action(
    id: 'wc_add_user_to_membership_plan',
    label: 'Add the user to a membership plan.',
    since: '1.0.0'
)]
/**
...
class AddUserToMembershipPlan extends AutomateAction
{
    ...
    public function register($actions)
    {
        $actions[$this->integration][$this->action] = [
            'label' => __('Add the user to a membership plan.', 'dollie'),
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        ...
        if (is_email($user_id)) {
                    ...
                    if (function_exists('wc_memberships_is_user_active_member')) {
                        if (true === $check_membership_plan && true === wc_memberships_is_user_active_member($user_id, $plan)) {
                            $error = [
                                ...
                                'response' => esc_attr__('This user has already an active membership in the specified membership plan.', 'dollie'),
            ...
            $error = [
                'status' => esc_attr__('Error', 'dollie'),
                'response' => esc_attr__('The user email is not valid.', 'dollie'),
