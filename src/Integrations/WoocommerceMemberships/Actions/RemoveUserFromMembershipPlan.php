...
namespace Dollie\SDK\Integrations\WoocommerceMemberships\Actions;
...
#[Action(
    id: 'wc_remove_user_from_membership_plan',
    label: 'Remove the user from a membership plan.',
    since: '1.0.0'
)]
/**
...
class RemoveUserFromMembershipPlan extends AutomateAction
{
    ...
    public function register($actions)
    {
        $actions[$this->integration][$this->action] = [
            'label' => __('Remove the user from a membership plan.', 'dollie'),
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        ...
        if (is_email($user_id)) {
            ...
            if ($user) {
                ...
                if ('-1' == $plan) {
                    ...
                    if (empty($user_all_memberships)) {
                        $error = [
                            ...
                            'response' => esc_attr__('The user was not a member of any membership plans.', 'dollie'),
                    ...
                    if (function_exists('wc_memberships_is_user_member')) {
                        ...
                        if (true !== $check_membership_plan) {
                            $error = [
                                'status' => esc_attr__('Error', 'dollie'),
                                'response' => esc_attr__('The user was not a member of the specified membership plan.', 'dollie'),
                ...
                $error = [
                    'status' => esc_attr__('Error', 'dollie'),
                    'response' => esc_attr__('This user is not registered.', 'dollie'),
