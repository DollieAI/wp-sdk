...
#[Action(
    id: 'create_user_if_not_exists',
    label: 'User: Create new user if not exists with Email',
    since: '1.0.0'
)]
/**
...
class CreateUserIfNotExists extends AutomateAction
{
    ...
    public function register($actions)
    {
        $actions[$this->integration][$this->action] = [
            'label' => __('User: Create new user if not exists with Email', 'dollie'),
        ...
        if ($user_id) {
            ...
            if (isset($user_arr['data'])) {
                if (empty($user_arr['data']->user_pass)) {
                    $user_arr['data']->user_pass = __('Unable to show the password while updating the user', 'dollie');
                } else {
