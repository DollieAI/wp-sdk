...
#[Action(
    id: 'update_user',
    label: 'User: Update User Details',
    since: '1.0.0'
)]
/**
 * UpdateUser.
...
class UpdateUser extends AutomateAction
{
    ...
    public function register($actions)
    {
        $actions[$this->integration][$this->action] = [
            'label' => __('User: Update User Details', 'dollie'),
            'action' => 'update_user',
            ...
            if ('user_login' === $meta_key) {
                if (! empty($meta_value)) {
                    if (! validate_username($meta_value)) {
                        wp_send_json_error(__('Invalid username: %1$s.', 'dollie'), $meta_value);
                    } else {
                        $user_id_has_this_id = username_exists($meta_value);

                        if ($user_id_has_this_id && $user_id_has_this_id !== $user_id) {
                            wp_send_json_error(__('Username "%1$s" already exists.', 'dollie'), $meta_value);
                        } else {
                ...
                if (! empty($meta_value)) {
                    if (! is_email($meta_value)) {
                        wp_send_json_error(__('Invalid email address: %1$s.', 'dollie'), $meta_value);
                    } else {

                        $user_id_has_email = email_exists($meta_value);

                        if ($user_id_has_email && $user_id_has_email !== $user_id) {
                            wp_send_json_error(__('Email address "%1$s" already exists.', 'dollie'), $meta_value);
                        } else {
