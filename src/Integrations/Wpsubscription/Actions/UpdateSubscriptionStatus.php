...
namespace Dollie\SDK\Integrations\Wpsubscription\Actions;
...
#[Action(
    id: 'wp_update_subscription_status',
...
class UpdateSubscriptionStatus extends AutomateAction
{
    ...
    public function register($actions)
    {
        $actions[$this->integration][$this->action] = [
            'label' => __('Update Subscription Status', 'dollie'),
            'action' => $this->action,
    ...
    public function _action_listener($user_id, $automation_id, $fields, $selected_options)
    {
        ...
        if (empty($subscription_id)) {
            return [
                'status' => 'error',
                'message' => __('Subscription ID is required.', 'dollie'),
        ...
        if (empty($new_status)) {
            return [
                'status' => 'error',
                'message' => __('Status is required.', 'dollie'),
        ...
        if (! $subscription || 'subscrpt_order' !== $subscription->post_type) {
            return [
                'status' => 'error',
                'message' => __('Invalid subscription ID.', 'dollie'),
        ...
        if (! in_array($new_status, $valid_statuses, true)) {
            return [
                'status' => 'error',
                'message' => __('Invalid status provided.', 'dollie'),
